<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&pause=1000&color=58A6FF&center=true&vcenter=true&width=500&lines=Hi+%F0%9F%90%8B+I'm+Santusht+Kotai;Backend+%26+Systems+Engineer;Founder+%40+Interleet;Building+Distributed+Systems" alt="Typing SVG" />
</h1>

<p align="center">
  <b>Building scalable backends, resilient cloud architecture, and high-performance developer tools.</b>
</p>

<p align="center">
  <a href="https://santusht.me"><img src="https://img.shields.io/badge/Website-100000?style=for-the-badge&logo=firefox&logoColor=white" alt="Website" /></a>
  <a href="https://www.linkedin.com/in/santusht-kotai-8a4454323/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://github.com/santusht06"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
  <a href="mailto:santushtkotai1221@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

<br/>

## 📌 Executive Summary

- 🚀 **Founder @ [Interleet](https://github.com/santusht06/interleet)** — Architecting next-gen developer productivity tools.
- ⚡ **Focus Areas** — Distributed Systems, Cloud Native (Kubernetes/AWS), Microservices & High-Throughput APIs.
- 🧪 **Current Stack Exploration** — Deep diving into low-latency **Rust**, concurrent **Go**, and cloud infrastructure.

---

## 🛠️ Technical Ecosystem

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=python,go,rust,ts,java,fastapi,nodejs,postgres,redis,mongodb,dynamodb,docker,kubernetes,aws,githubactions,linux&perline=8" alt="My Skills" />
  </a>
</p>

<details>
<summary><b>Detailed Tech Breakdown</b></summary>
<br/>

| Category | Stack & Tools |
|---|---|
| **Core Languages** | `Python` · `Go` · `Rust` · `TypeScript` · `Java` · `SQL` |
| **Backend & Distributed Systems** | `FastAPI` · `Gin` · `Node.js` · `gRPC` · `WebSockets` · `REST APIs` |
| **Infrastructure & DevOps** | `Docker` · `Kubernetes` · `AWS (EKS, EC2, RDS)` · `GitHub Actions` · `Linux` |
| **Data & Storage** | `PostgreSQL` · `Redis` · `MongoDB` · `Qdrant` · `DynamoDB` |

</details>

---

## ⚙️ Engineering Principles

- 🎯 **Simplicity First** — Less code = fewer points of failure. Keep abstractions lean and intent clear.
- 🔒 **Strong Contracts** — Robust API design, deterministic validation, and strict boundary typing.
- 📈 **Performance & Resilience** — Test early, profile bottlenecks, and build self-healing systems.

---

## 📊 Activity & Analytics

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/santusht06/santusht06/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/santusht06/santusht06/output/github-contribution-grid-snake.svg" />
    <img alt="GitHub contribution snake" src="https://raw.githubusercontent.com/santusht06/santusht06/output/github-contribution-grid-snake-dark.svg" width="100%" />
  </picture>
</div>

<br/>

<div align="center">
  <a href="https://github.com/santusht06">
    <img height="165px" src="https://github-readme-stats-eight-theta.vercel.app/api?username=santusht06&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=58A6FF&icon_color=58A6FF&text_color=c9d1d9&ring_color=58A6FF&count_private=true&include_all_commits=true" alt="Santusht's GitHub Stats" />
    <img height="165px" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=santusht06&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=58A6FF&text_color=c9d1d9&langs_count=6" alt="Top Languages" />
  </a>
</div>
